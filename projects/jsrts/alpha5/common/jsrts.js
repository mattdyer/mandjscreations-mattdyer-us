	...
	this.ShowGameForm = function(){
		...
		$.post('ajax/index.php',{'Action':'GetGames'},function(data){
			...
			if(data.length > 0){
				...
				JoinButton.click(function(){
					...
					$.post('ajax/index.php',{'Action':'CreatePlayer','GameID':Settings.GameID,'Name':PlayerName},function(data){
						//alert(PlayerID);
						Settings.ActivePlayerID = data.PlayerID;
						that.addPlayer('Queue',{'PlayerID':data.PlayerID,'Name':PlayerName,'Color':data.Color});
						that.StartGame();
					});
				});
		...
		$('#CreateGameButton').click(function(){
			...
			$.post('ajax/index.php',{'Action':'CreateGame','Name':GameName,'Started':'No'},function(GameID){
				...
				$.post('ajax/index.php',{'Action':'CreatePlayer','GameID':GameID,'Name':PlayerName},function(data){
					//alert(PlayerID);
					Settings.ActivePlayerID = data.PlayerID;
					that.addPlayer('Queue',{'PlayerID':data.PlayerID,'Name':PlayerName,'Color':data.Color});
					that.StartGame();
		...
		this.UnitDiv.mouseover(function(event){
			...
			for(PlayerIndex in Players){
				...
				if(Players[PlayerIndex].Info.PlayerID == $(event.target).data().Unit.PlayerID){	
					$(this).css('border-bottom','1px solid ' + Players[PlayerIndex].Info.Color);
				}
			}
