function JSRTS(Settings){
	...
	this.ShowGames = function(){
		$.post('ajax/index.php',{'Action':'GetGames'},function(data){
			...
			if(data.length > 0){
				...
				JoinForm.html('');
				JoinForm.append('<h3>Join a Game</h3>');
				JoinForm.append('<div>No Games Available</div>');
			}
	...
	this.checkForPlayers = function(){
		...
		$.post('ajax/index.php',{'Action':'GetPlayers','GameID':Settings.GameID},function(PlayerArray){
			...
			if($.type(PlayerArray) == 'string'){
				if(PlayerArray == 'Start'){
					that.StartGame();
				}
				if(PlayerArray == 'Cancelled'){
	...
	this.LeaveGame = function(){
		$.post('ajax/index.php',{'Action':'LeaveGame','PlayerID':that.getActivePlayerID(),'GameCreator':Settings.GameCreator},function(data){
	...
	this.StartGame = function(){
		...
		Settings.PlayerBar = $('<div class="PlayerBar"></div>');
		Settings.PlayerBar.css({'width':Settings.width,'height':'30px'});
		
		Settings.GameArea.before(Settings.PlayerBar);
		...
		setTimeout(function(){
			...
			for(var i in Players){
				...
				PlayerDiv.html('Player ' + PlayerCount + ': ' + Player.Name);
				PlayerDiv.css({'display':'inline-block','color':Player.Color,'padding':'5px'});
				Settings.PlayerBar.append(PlayerDiv);
				//alert(Player.Color);
			}
		},1000);
	}
	...
	var Player = function(Startup){
		for(var param in Startup){
			this[param] = Startup[param];
		}
		//this.Info = Startup;
	}
	...
	var Unit = function(Startup,Specs){
		...
		this.UnitDiv.mouseover(function(event){
			...
			for(PlayerIndex in Players){
				...
				if(Players[PlayerIndex].PlayerID == $(event.target).data().Unit.PlayerID){	
					$(this).css('border-bottom','3px solid ' + Players[PlayerIndex].Color);
				}
			}
